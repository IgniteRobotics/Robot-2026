...
public class VisionSubsystem extends SubsystemBase {
    ...
    private void evaluateMeasurement(EstimatedRobotPose pose, String cameraName){
        ...
        double poseDistance = pose.estimatedPose.toPose2d().getTranslation().getDistance(driveStats.Pose.getTranslation());
        ...
        if(driveStats.Speeds.omegaRadiansPerSecond > Math.PI) {
            return;
        } else if (driveStats.Speeds.omegaRadiansPerSecond > 0.5){ //if you're rotating quickly, trust the pose less
            thetaStd = 0.75;
        }
