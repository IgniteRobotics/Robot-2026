...
config = StreamConfig(
    stream_output=["head_photon"], # Uncomment and check your stream output name
    data_output=["top_class","classification_predictions","detection_predictions"]      # Get prediction data via datachannel,
...
session = client.webrtc.stream(
    source=source,
    workflow="classify-and-conditionally-detect",
    workspace="myspace-rppeb",
    image_input="image",
    config=config
)
